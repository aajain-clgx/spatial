...
var userlib = function(conf){
    ...
    function insertRequestObject(publickey, request, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'INSERT INTO userapi_jobrequest(PublicKey, Request) VALUES (?,?)';

      connection.query(mysqlQuery, [publickey, JSON.stringify(request, null)], function(err, result){
        connection.end();
        if(err){
          return callback(new Exception.Mysql(err), null);
        }else{
    ...
    function getRequestObject(publickey, requestid, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'SELECT Request from userapi_jobrequest where PublicKey = ? and ID = ?';
      
      connection.query(mysqlQuery, [publickey, requestid], function(err, rows){
        connection.end();
        if(err){
          return callback(new Exception.Mysql(err), null);
        }else{
          if(rows.length === 0){
            return callback(new Exception.NoRequestObjFound(publickey, requestid), null);       
          }else{
            return callback(null, rows[0].Request);
          }
        }
      });
    }

    function deleteRequestObject(publickey, requestid, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'DELETE FROM userapi_jobrequest where PublicKey = ? and ID = ?';
      
      connection.query(mysqlQuery, [publickey, requestid], function(err, result){
        connection.end();
        if(err){
          return callback(new Exception.Mysql(err), null);
        }else{
    ...
    function updateRequestObject(publickey, requestid, request, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'UPDATE userapi_jobrequest SET request = ? where PublicKey = ? and ID = ?';
      
      connection.query(mysqlQuery, [JSON.stringify(request, null), publickey, requestid], function(err, result){
        connection.end();
        if(err){
          return callback(new Exception.Mysql(err), null);
        }else{
    ...
    function getPrivateKey(publickey, callback){
        if(!_initialized){
          ...
          getPermissionFromMysql(function(err, rows){
            if(err){
              ...
              if(lookup[publickey] === undefined){
                ...
                return callback(null, lookup[publickey].PrivateKey);
              }
            }
          ...
          // Get it from Sqlite
          _sqlitedb.all("SELECT PrivateKey from  UserPermissions where PublicKey = ?", publickey, function(err, rows){
            if(err){
              return callback(new Exception.Sqlite(err), null);
            }else{
              if(rows.length === 0){
                ...
                return callback(null, rows[0].PrivateKey);
              }
            }
          });
        }
    ...
    return {
      ...
      insertRequestObject: insertRequestObject
    };
