...
var userlib = function(conf){
    ...
    function insertRequestObject(publickey, request, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'INSERT INTO userapi_request(PublicKey, Request) VALUES (?,?)';

      connection.query(mysqlQuery, [publickey, request], function(err, result){
        connection.end();
        if(err){
          return callback(new Exception.Mysql(err), null);
        }else{
    ...
    function getRequestObject(publickey, requestid, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'SELECT Request from userapi_request where PublicKey = ? and ID = ?';
    ...
    function deleteRequestObject(publickey, requestid, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'DELETE FROM userapi_request where PublicKey = ? and ID = ?';
      
      connection.query(mysqlQuery, [publickey, requestid], function(err){
        connection.end();
        if(err){
          return callback(new Exception.Mysql(err), null);
        }else{
    ...
    function updateRequestObject(publickey, requestid, request, callback){
      var connection = mysql.createConnection(_webusageConf);
      var mysqlQuery = 'UPDATE userapi_request SET request = ? where PublicKey = ? and ID = ?';
      
      connection.query(mysqlQuery, [request, publickey, requestid], function(err){
        connection.end();
        if(err){
          return callback(new Exception.Mysql(err), null);
        }else{
