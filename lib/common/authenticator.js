...
function Authenticator() {
  ...
  this.saltKey = function(key, salt) {
  ...
  // Returns true if the signature generated from the required portion of the
  ...
  this.validateRequest = function(urlString, salt, privateKey) {
    ...
    if (parametersValid) {
      ...
      matchForSignature = urlString.match(_sigRegExp);
    ...
    if (timestampValid) {
      ...
      var saltedKey = this.saltKey(privateKey, salt);
      var signature = matchForSignature.toString().split(_sigReplaceRegExp)[1];
