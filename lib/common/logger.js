...
function Logger() {
  ...
  var levelIsDefined = function(level) {
    var values = LoggingLevels.enums;
    for (var i = 0; i < values.length; i++) {
      if (values[i].has(level)) {
        return true;
      }
    }
    return false;
  ...
  this.setFileLevel = function(level) {
    if (levelIsDefined(level)) {
      _fileLevel = LoggingLevels.get(level);
    }
  };

  this.setEmailLevel = function(level) {
    if (levelIsDefined(level)) {
      _emailLevel = LoggingLevels.get(level);
    }
  };
